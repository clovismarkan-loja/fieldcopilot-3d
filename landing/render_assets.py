...
# Assets: hero360 hero_zoom conjunto360 camera_zoom bateria_giro lanterna fone_encaixe montagem celular_giro final_perfil
# (todos importam os GLBs v4; celular_giro substituiu o relogio_giro)
import bpy
import math
import sys
import os
...
PROVA = "prova" in sys.argv   # so alguns quadros p/ conferir enquadramento
...
def melhora_materiais():
    ...
    for m in bpy.data.materials:
        ...
        elif n.startswith("elastico"):
            nt = m.node_tree
            _set(b, "Roughness", 0.93)
            _set(b, "Sheen Weight", 0.22)
            # v4: banda/cinta ja trazem normal map de trama no GLB — nao sobrescrever
            if any(x.type == 'NORMAL_MAP' for x in nt.nodes) or "TramaBump" in [x.label for x in nt.nodes]:
                continue
...
def orbita_seq(nome, alvo, raio0, raio1, elev0, elev1, nf, lens=70, ang0=20.0, giro=360.0):
    ...
    quadros = [0, nf // 2, nf - 1] if PROVA else range(nf)
    for f in quadros:
        t = f / (nf - 1)
...
if ASSET == "hero360":
    ...
    os.makedirs(pasta, exist_ok=True)
    # v4 (medido no GLB): lente do modulo camera em (0.0640, -0.0827, 0.0250)
    POS_DIR = Vector((0.0358, -0.0827, 0.025))
    ...
    p1, a1 = Vector((0.085, -0.052, 0.012)) + POS_DIR, Vector((0.028, -0.002, 0.0)) + POS_DIR
    # aproximacao final: quase colado na lente -> quadro praticamente preto
    p2, a2, l2 = Vector((0.0770, -0.0857, 0.0255)), Vector((0.0640, -0.0827, 0.0250)), 95.0
    NF = 120
    ...
    importa("novo_modulo_camera_v4.glb")
    luz_estudio(0.6)
    setup_render()
    cam = cam_nova(70)
    pasta = os.path.join(OUT, "seq_camera_zoom")
    os.makedirs(pasta, exist_ok=True)
    # v4: corpo -0.094..0.030 (centro -0.033); lente em (0.0282, 0, 0)
    p0, a0, l0 = Vector((0.21, -0.24, 0.13)), Vector((-0.033, 0.0, 0.0)), 70.0
    p1, a1, l1 = Vector((0.085, -0.052, 0.012)), Vector((0.028, -0.002, 0.0)), 95.0
    quadros = [0, 45, 89] if PROVA else range(90)
    for f in quadros:
        t = smoothstep(f / 89.0)
        cam.data.lens = l0 + (l1 - l0) * t
        mira(cam, p0.lerp(p1, t), a0.lerp(a1, t))
        render_para(os.path.join(pasta, f"f{f:03d}"))
    print("OK seq_camera_zoom:", len(list(quadros)), "frames")

elif ASSET == "bateria_giro":
    importa("modulo_bateria_v4.glb")
    luz_estudio(0.6)
    setup_render()
    # termina exatamente no enquadramento dos stills da lanterna (continuidade do cross-fade)
    # v4: corpo -0.094..0.027 (mais longo); lanterna na ponta +X (LED x=0.021)
    orbita_seq("seq_bateria_giro", (-0.02, 0.0, 0.0), 0.32, 0.26, math.radians(18), math.radians(16.9), 90, lens=70, ang0=-47.6, giro=360)

elif ASSET == "lanterna":
    objs = importa("modulo_bateria_v4.glb")
    luz_estudio(0.6)
    setup_render()
    ...
    for m in bpy.data.materials:
        if m.name.startswith("LED") and m.use_nodes:
            mat_led = m
    # mesma camera do frame final do seq_bateria_giro (alvo -0.02, raio 0.26, elev 16.9, ang -47.6)
    cam = cam_nova(70)
    _r, _e, _a = 0.26, math.radians(16.9), math.radians(-47.6)
    _loc = (-0.02 + _r * math.cos(_a) * math.cos(_e), _r * math.sin(_a) * math.cos(_e), _r * math.sin(_e))
    mira(cam, _loc, (-0.02, 0.0, 0.0))
    ...
    # feixe fake: spot saindo do LED (v4: LED em (0.021, 0.002, 0), aponta +X)
    bpy.ops.object.light_add(type='SPOT', location=(0.027, 0.002, 0.0))
    ...
    mod_objs = importa("novo_modulo_camera_v4.glb")   # v4 ja traz o Boss_P3 embaixo
    fone_objs = importa("fone_gancho.glb")
    ee = bpy.data.objects.new("G_Fone", None)
    scene.collection.objects.link(ee)
    for o in fone_objs:
        o.parent = ee
    # modulo tambem anima: desce de cima ate encontrar o plug que sobe
    gm = bpy.data.objects.new("G_Mod", None)
    scene.collection.objects.link(gm)
    for o in mod_objs:
        if o.parent is None:
            o.parent = gm
    # v4: Boss_P3 em (-0.0152, -0.0020), base z = -0.0254
    POS_FIM = Vector((-0.0152, -0.0020, -0.0470))
    ...
    mira(cam, (0.1116, -0.156, -0.02), (-0.0134, -0.016, -0.035))
    ...
    os.makedirs(pasta, exist_ok=True)
    quadros = [0, 30, 59] if PROVA else range(60)
    for f in quadros:
        t = smoothstep(f / 59.0)
        ...
        render_para(os.path.join(pasta, f"f{f:03d}"))
    print("OK seq_montagem:", len(list(quadros)), "frames")

elif ASSET == "celular_giro":
    # substitui o relogio_giro (reformulacao: celular Cubot na braçadeira de braco)
    importa("celular_braco.glb")
    luz_estudio(0.6)
    setup_render()
    ...
    importa("headset_montado_v4.glb")
    # esfera preta como um planeta escuro: so a luz de recorte desenha o volume
    matp = bpy.data.materials.new("PlanetaEscuro")
    matp.use_nodes = True
    bt = matp.node_tree.nodes["Principled BSDF"]
    bt.inputs["Base Color"].default_value = (0.012, 0.012, 0.014, 1.0)
    bt.inputs["Roughness"].default_value = 0.42
    ...
    cam = cam_nova(85)
    # esfera centralizada; camera orbitada ~20 graus p/ tras => modulo desliza p/ a direita da faixa
    mira(cam, (-0.109, -0.618, 0.05), (0.01, 0.0, 0.02))
    pasta = os.path.join(OUT, "stills")
    os.makedirs(pasta, exist_ok=True)
